...
async def cb_navg(bot, update: CallbackQuery):
    ...
    if ((index_val + 1 )== max_pages) or ((index_val + 1) == len(results)): # Max Pages
        ...
        temp_results.append([
            InlineKeyboardButton("🍁 𝔹𝔸C𝕂 P𝔸𝔾𝔼 🍁", callback_data=f"navigate({index_val}|back|{query})"),
            InlineKeyboardButton("🍁 ℕ𝔼𝕏𝕋 P𝔸𝔾𝔼 🍁", callback_data=f"navigate({index_val}|next|{query})")
        ])
    ...
    text=f"<i>🎥𝐆𝐫𝐨𝐮𝐩:- @Film_House2</i> \n 🎬𝐑𝐞𝐪𝐮𝐞𝐬𝐭𝐞𝐝 𝐌𝐨𝐯𝐢𝐞:- <code>{query}</code> \n 😇𝐑𝐞𝐬𝐮𝐥𝐭𝐬 𝐅𝐨𝐮𝐧𝐝:- <code>{leng}</code> \n ⚡️𝐑𝐞𝐪𝐮𝐞𝐬𝐭𝐞𝐝 𝐁𝐲:- {} /n ⭕️𝗣𝗿𝗲𝘀𝘀 𝗧𝗵𝗲 𝗗𝗼𝘄𝗻 𝗕𝘂𝘁𝘁𝗼𝗻𝘀 𝗧𝗼 𝗔𝗰𝗰𝗲𝘀𝘀 𝗧𝗵𝗲 𝗙𝗶𝗹𝗲⭕️
